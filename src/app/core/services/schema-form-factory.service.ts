...
export class SchemaFormFactory {
  ...
  private shouldIncludeField(
    col: ColumnInfo,
    mode: FormMode,
    _pkColumns: ReadonlySet<string>,
  ): boolean {
    const autoGenerated = this.isAutoGenerated(col);
    if (autoGenerated && col.identity_generation === 'ALWAYS') {
      return false;
    }
    if (mode === 'add' && autoGenerated) {
      return false;
    }
    return true;
  }
  ...
  private columnToField(
    ...
    const isReadOnly = (isPk || this.isAutoGenerated(col)) && mode === 'edit';

    const isFk = fkMap.has(col.column_name);

    const field: FormFieldDefinition = {
      id: col.column_name,
      title: this.humanize(col.column_name),
      component: isFk ? 'fk' : this.mapComponent(col),
  ...
  private buildConfig(
    ...
    if (fk) {
      ...
      config['referencedTable'] = fk.referencedTable;
      config['referencedColumn'] = fk.referencedColumn;
    }
